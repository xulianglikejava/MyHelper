...
public class 单人业原火 {
    ...
    public void 开始业原火() {
        ...
        try {
            // 读取判断点信息
            ArrayList<int[]> 挑战的坐标 = mouseUtils.FileToArrayList("单人业原火坐标.txt");
            ...
            while (count <= maxCount) {
                ...
                for (int i = 0; i < 挑战的坐标.size(); i++) {
                    ...
                    // 等打完
                    System.out.println( (int) playTime + Math.random() * 3000);
                    Thread.sleep((int) (playTime + Math.random() * 4000));
                    /*再点击结算*/
                    mouseUtils.MouseResponse(挑战的坐标.get(i));
                    // 结算完等待开始
                    Thread.sleep((int) (Math.random() * waitTime + 200));
                }
                count++;
    ...
    @Test
    public void 设置单人业原火坐标() {
        System.out.println("开始启动小助手...");
        System.out.println("设置坐标...");
        try {
            mouseUtils.saveMouseInfo("单人业原火坐标.txt");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
